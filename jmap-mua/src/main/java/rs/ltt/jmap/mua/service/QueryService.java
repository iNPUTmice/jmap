...
public class QueryService extends AbstractMuaService {
        ...
        return Futures.transformAsync(
                ...
                queryStateWrapper -> {
                    ...
                    if (!queryStateWrapper.canCalculateChanges || queryStateWrapper.upTo == null) {
                        ...
                        Preconditions.checkNotNull(
                                queryStateWrapper.objectsState.threadState,
                                "threadState can not be null if queryState was not");
    ...
    private ListenableFuture<Status> query(
        ...
        if (queryStateWrapper.upTo == null || !afterEmailId.equals(queryStateWrapper.upTo.id)) {
            ...
            // the last item in
            // a query. This will probably fix itself once the update command has run as well as a
            // subsequent updateQuery() call.
            throw new InconsistentQueryStateException(
                    "upToId from QueryState needs to match the supplied afterEmailId");
        }
    ...
    private ListenableFuture<Status> refreshQuery(
        ...
        return Futures.transformAsync(
                queryChangesResponsesFuture,
                methodResponses -> {
                    QueryChangesEmailMethodResponse queryChangesResponse =
                    ...
                    //  2) store query results; If query cache sees an outdated email state it will
                    // fail
                    ...
                    // it might be that a previous fetchMissing() has failed. so better safe than
                    // sorry
    ...
    private void registerInvalidateQueryCacheCallback(
                    ...
                    public void onFailure(@Nonnull Throwable throwable) {
                        if (MethodErrorResponseException.matches(throwable, methodError)) {
                            if (evaluateAdditionalCondition(additionalCondition)) {
                                ...
                                LOGGER.info(
                                        "Not invalidating QueryCache after {} additional because"
                                                + " condition was false",
                                        methodError);
                            }
                        }
    ...
    private ListenableFuture<Status> initialQuery(
        ...
        // these need to be processed *before* the Query call or else the fetchMissing will not
        // honor newly fetched ids
        final List<ListenableFuture<Status>> piggyBackedFuturesList =
                getService(RefreshService.class).refresh(queryStateWrapper.objectsState, multiCall);
        ...
        return Futures.transformAsync(
                ...
                queryResult -> {
                    ...
                    //  3) store query results; If query cache sees an outdated email state it will
                    // fail
    ...
    private ListenableFuture<Status> fetchMissing(final Missing missing) {
        ...
        final JmapClient.MultiCall multiCall = jmapClient.newMultiCall();
        final ListenableFuture<Status> updateThreadsFuture =
